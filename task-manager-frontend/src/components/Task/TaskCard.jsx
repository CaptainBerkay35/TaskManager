...
function TaskCard({ task, onEdit, onDelete, onToggleComplete }) {
  ...
  return (
    <>
      <div 
        className={`bg-white p-4 rounded-lg shadow hover:shadow-lg transition border-2 cursor-pointer ${
          isCompleted 
          ...
          <div className="flex-1">
            ...
            {task.dueDate && (
              <p className={`text-xs mb-2 ${
                ...
                {new Date(task.dueDate) < new Date() && !isCompleted && "⚠ Gecikmiş: "}
                Son Teslim Tarihi: {new Date(task.dueDate).toLocaleDateString("tr-TR", {
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                })}
